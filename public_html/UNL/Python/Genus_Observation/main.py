...
def get_image_file_name(adic, df):
    for index, row in df.iterrows():
        ...
        if ImageName not in adic[Genus][Detail]:
            adic[Genus][Detail].append(ImageName)
    return adic

def writeDict2Json(adict):
    json_object = json.dumps(adict, indent=4)
    fn = "../../../files/json/unl/Genus_Observation.json"
