...
def get_genus(adic, df):
    for index, row in df.iterrows():
        ...
        adic[View][Order][Family][Genus] = list()
    return adic
...
def get_image_file_name(adic, df):
    ...
    for index, row in df.iterrows():
        ...
        if ImageName not in adic[View][Order][Family][Genus]:
            adic[View][Order][Family][Genus].append(ImageName)
