...
def get_image_file_name(adic, df):
    for index, row in df.iterrows():
        ...
        if ImageName not in adic[Class][Detail]:
            adic[Class][Detail].append(ImageName)
    return adic

def writeDict2Json(adict):
    json_object = json.dumps(adict, indent=4)
    fn = "../../../files/json/unl/Class_Observation.json"
